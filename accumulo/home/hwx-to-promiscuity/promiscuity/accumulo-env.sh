...
test -z "$JAVA_HOME"             && export JAVA_HOME=/home/elserj/.gentoo/java-config-2/current-user-vm
...
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xmx4g -Xms1g -XX:NewSize=500m -XX:MaxNewSize=500m "
#test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xmx4g -Xms1g -XX:NewSize=500m -XX:MaxNewSize=500m -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=8989"
